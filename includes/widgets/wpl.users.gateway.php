...
function wpl_process_login_new_users_gateway( $provider, $redirect_to, $hybridauth_user_profile )
{
	// HOOKABLE:
	do_action( "wpl_process_login_new_users_gateway_start", $provider, $redirect_to, $hybridauth_user_profile );

	$assets_base_url = WORDPRESS_PIXELPIN_LOGIN_PLUGIN_URL . 'assets/img/16x16/';

	// remove wpl widget
	remove_action( 'register_form', 'wpl_render_auth_widget_in_wp_register_form' );
	...
	$requested_user_login        = apply_filters( 'wpl_new_users_gateway_alter_requested_login', $requested_user_login );
	...
	$linking_enabled = get_option( 'wpl_settings_bouncer_accounts_linking_enabled' );
        $require_email   = get_option( 'wpl_settings_bouncer_profile_completion_require_email' );
        $change_username = get_option( 'wpl_settings_bouncer_profile_completion_change_username' );
        $extra_fields    = get_option( 'wpl_settings_bouncer_profile_completion_hook_extra_fields' );

	if( isset( $_REQUEST["bouncer_account_linking"] ) )
	{
		if( $linking_enabled == 2 )
		{
			return wpl_process_login_render_notice_page( _wpl__( "Not tonight.", 'wordpress-pixelpin-login' ) );
		}
		...
		else
		{
			...
			*/
			...
			if( $require_email == 1 )
			{
				...
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Please type your e-mail address.', 'wordpress-pixelpin-login' );
				}

				if ( ! is_email( $requested_user_email ) )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Please enter a valid email address.', 'wordpress-pixelpin-login' );
				}

				if ( wpl_wp_email_exists( $requested_user_email ) )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Sorry, that email address is already used!', 'wordpress-pixelpin-login' );
				}
			}
			# }}} validate usermail

			# {{{ validate username (called login in wpl)
			if( $change_username == 1 )
			{
				$illegal_names = array(  'www', 'web', 'root', 'admin', 'main', 'invite', 'administrator' );

				$illegal_names = apply_filters( 'wpl_new_users_gateway_alter_illegal_names', $illegal_names );

				if ( in_array( $requested_user_login, $illegal_names ) == true )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: That username is not allowed.', 'wordpress-pixelpin-login' );
				}

				if ( strlen( $requested_user_login ) < 4 )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Username must be at least 4 characters.', 'wordpress-pixelpin-login' );
				}

				if ( strpos( ' ' . $requested_user_login, '_' ) != false )
				{
					// $profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Sorry, usernames may not contain the character &#8220;_&#8221;!', 'wordpress-pixelpin-login' );
				}

				if ( preg_match( '/^[0-9]*$/', $requested_user_login ) )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Sorry, usernames must have letters too!', 'wordpress-pixelpin-login' );
				}

				if ( username_exists( $requested_user_login) )
				{
					$profile_completion_errors[] = _wpl__( '<strong>ERROR</strong>: Sorry, that username already exists!', 'wordpress-pixelpin-login' );
				}
			}
			# }}} validate username
			...
			$profile_completion_errors = apply_filters( 'wpl_new_users_gateway_alter_profile_completion_errors', $profile_completion_errors );
	...
	if( $shall_pass == false )
	{
		$provider_name = wpl_get_provider_name_by_id( $provider );
?>
<!DOCTYPE html>
	<head>
		...
		<div id="login">
			<div id="login-panel">
				<div id="welcome">
					...
					<p><?php printf( _wpl__( "You're now signed in with your %s account but you are still one step away of getting into our website", 'wordpress-pixelpin-login' ), $provider ); ?>.</p>
				...
				<table id="mapping-options" border="0">
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<h4><?php _wpl_e( "Already have an account", 'wordpress-pixelpin-login' ); ?>?</h4>
								<p style="font-size: 12px;"><?php printf( _wpl__( "Link your existing account on our website to your %s ID.", 'wordpress-pixelpin-login' ), $provider ); ?></p>
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<h4><?php _wpl_e( "New to our website", 'wordpress-pixelpin-login' ); ?>?</h4>
							<p style="font-size: 12px;"><?php printf( _wpl__( "Create a new account and it will be associated with your %s ID.", 'wordpress-pixelpin-login' ), $provider ); ?></p>
						</td>
					</tr>
					...
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<input type="button" value="<?php _wpl_e( "Link my account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="display_mapping_authenticate();" >
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<?php if( $require_email != 1 && $change_username != 1 && $extra_fields != 1 ): ?>
                                <input type="button" value="<?php _wpl_e( "Create a new account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="document.getElementById('info-form').submit();" >
							<?php else : ?>
                                <input type="button" value="<?php _wpl_e( "Create a new account", 'wordpress-pixelpin-login' ); ?>" class="button-primary" onclick="display_mapping_complete_info();" >
                            <?php endif; ?>
						</td>
					</tr>
				</table>
				...
				<form method="post" action="<?php echo site_url( 'wp-login.php', 'login_post' ); ?>" id="link-form">
					<table id="mapping-authenticate" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _wpl__( "Please enter your username and password of your existing account on our website. Once verified, it will linked to your % ID", 'wordpress-pixelpin-login' ), ucfirst( $provider ) ) ; ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
								<label>
									<?php _wpl_e( "Username", 'wordpress-pixelpin-login' ); ?>
									<br />
									<input type="text" name="user_login" class="input" value=""  size="25" placeholder="" />
								</label>

								<label>
									<?php _wpl_e( "Password", 'wordpress-pixelpin-login' ); ?>
									<br />
									<input type="text" name="user_password" class="input" value="" size="25" placeholder="" />
								</label>

								<input type="submit" value="<?php _wpl_e( "Continue", 'wordpress-pixelpin-login' ); ?>" class="button-primary" >

								<a href="javascript:void(0);" onclick="display_mapping_options();" class="back-to-options"><?php _wpl_e( "Back", 'wordpress-pixelpin-login' ); ?></a>
							</td>
						</tr>
					</table>
					...
					<input type="hidden" id="action" name="action" value="wordpress_pixelpin_account_linking">
					...
					<table id="mapping-complete-info" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _wpl__( "Please fill in your information in the form below. Once completed, you will be able to automatically sign into our website through your %s ID", 'wordpress-pixelpin-login' ), $provider_name ); ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
                                ...
                                <?php if( $require_email == 1 ): ?>
                                    <label>
                                        <?php _wpl_e( "E-mail", 'wordpress-pixelpin-login' ); ?>
                                        <br />
			...
			<p class="back-to-home">
				<a href="<?php echo home_url(); ?>">&#8592; <?php printf( _wpl__( "Back to %s", 'wordpress-pixelpin-login' ), get_bloginfo('name') ); ?></a>
			</p>
		</div>

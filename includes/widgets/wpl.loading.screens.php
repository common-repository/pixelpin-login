...
*/
if( ! function_exists( 'wpl_render_redirect_to_provider_loading_screen' ) )
{
	function wpl_render_redirect_to_provider_loading_screen( $provider )
	{
...
<!DOCTYPE html>
	<head>
		...
		<title><?php _wpl_e("Redirecting...", 'wordpress-pixelpin-login') ?> - <?php bloginfo('name'); ?></title>
		<style type="text/css">
			html {
				background: #f1f1f1;
			}
	...
	<body id="loading-screen" onload="init();">
		<table width="100%" border="0">
			...
			<tr>
				<td align="center">
					<div>
						<?php echo sprintf( _wpl__( "Contacting <b>%s</b>, please wait...", 'wordpress-pixelpin-login'), _wpl__( ucfirst( $provider ), 'wordpress-pixelpin-login') )  ?>
					</div>
				</td>
			</tr>
		</table>
...
*/
if( ! function_exists( 'wpl_render_return_from_provider_loading_screen' ) )
{
	function wpl_render_return_from_provider_loading_screen( $provider, $authenticated_url, $redirect_to, $wpl_settings_use_popup )
	{
		/*
		* If Authentication displayis undefined or eq Popup ($wpl_settings_use_popup==1)
		* > create a from with javascript in parent window and submit it to wp-login.php ($authenticated_url)
		* > with action=wordpress_pixelpin_authenticated, then close popup
		*
		* If Authentication display eq In Page ($wpl_settings_use_popup==2)
		* > create a from in page then submit it to wp-login.php with action=wordpress_pixelpin_authenticated
		*/
...
<!DOCTYPE html>
	<head>
		...
		<script>
			...
			{
				<?php
					if( $wpl_settings_use_popup == 1 || ! $wpl_settings_use_popup ){
						?>
							if( window.opener )
							{
								window.opener.wpl_wordpress_pixelpin_login({
									'action'   : 'wordpress_pixelpin_authenticated',
					...
					elseif( $wpl_settings_use_popup == 2 ){
						?>
							document.loginform.submit();
						<?php
					}
	...
	<body id="loading-screen" onload="init();">
		<table width="100%" border="0">
			...
			<tr>
				<td align="center">
					<div>
						<?php echo _wpl_e( "Processing, please wait...", 'wordpress-pixelpin-login');  ?>
					</div>
				</td>
			</tr>
		</table>
		...
		<form name="loginform" method="post" action="<?php echo $authenticated_url; ?>">
			...
			<input type="hidden" id="action" name="action" value="wordpress_pixelpin_authenticated">
		</form>
	</body>
</html>
<?php

...
function wpl_get_version()
{
	...
	return $WORDPRESS_PIXELPIN_LOGIN_VERSION;
}
...
function wpl_get_current_url()
{
	...
	// HOOKABLE:
	$current_url = apply_filters( 'wpl_hook_alter_current_url', $current_url );
...
*/
function wpl_display_dev_mode_debugging_area( $keyword = 'wpl_' )
{
	global $wpdb, $wp_actions , $wp_filter;
?>
<style>
	.wpl-dev-nonselectsql {
		color: #a0a !important;
	}
	.wpl-dev-expensivesql {
		color: #f44 !important;
	}
	.wpl-dev-optionfunc {
		color: #4a4 !important;
	...
	.wpl-dev-usedhook, .wpl-dev-usedhook a {
		color: #1468fa;
	}
	.wpl-dev-usedwplhook {
		color: #a0a !important;
	}
	.wpl-dev-unusedhook, .wpl-dev-unusedhook a{
		color: #a3a3a3 !important;
	}
	.wpl-dev-hookcallback, .wpl-dev-hookcallback a {
		color: #4a4 !important;
	}
	.wpl-dev-table {
		width:100%;
	...
	.wpl-dev-table td, .wpl-dev-table th {
...
?>
					...
					if( stristr( $caller_name, $keyword ) || stristr( $sql, $keyword ) || stristr( $stack,$keyword ) )
					{
						?>
							<tr>
								...
								<td valign="top" class="<?php if( ! stristr( '#' . $sql, '#select ' ) ) echo 'wpl-dev-nonselectsql'; ?>"><?php echo nl2br( $sql ); ?></td>
								<td valign="top" width="50" nowrap class="<?php if( $time > 0.05 ) echo 'wpl-dev-expensivesql'; ?>"><?php echo number_format( $time, 4, '.', '' ); ?></td>
							</tr>
						<?php

						$total_wpl_queries++;
						$total_wpl_queries_time += $time;
					}
				}
			}
		?>
								...
								foreach( $callbacks as $callback )
								{
									if( isset( $callback['function'] ) && is_string( $callback['function'] ) )
									{
										if( stristr( $callback['function'], $keyword ) || stristr( $name, $keyword ) )
										{
											?>
												<tr>
													<td valign="top" width="270" nowrap class="wpl-dev-usedhook">
														<?php
													...
													<td valign="top" class="wpl-dev-hookcallback">
														<?php
	...
	<table class="wpl-dev-table">
		<tbody>
			...
			<tr><th><label>Plugin path</label></th><td><?php echo WORDPRESS_PIXELPIN_LOGIN_ABS_PATH; ?></td></tr>
			<tr><th><label>Plugin url</label></th><td><?php echo WORDPRESS_PIXELPIN_LOGIN_PLUGIN_URL; ?></td></tr>
			<tr><th><label>HA endpoint</label></th><td><?php echo WORDPRESS_PIXELPIN_LOGIN_HYBRIDAUTH_ENDPOINT_URL; ?></td></tr>
		</tbody>
	</table>
...
*/
function wpl_generate_backtrace()
{
    $e = new Exception();
    $trace = explode( "\n", $e->getTraceAsString() );
    ...
    for ( $i = 0; $i < $length; $i++ )
    {
        $result[] = ( $i + 1 )  . ')' . str_ireplace( array( realpath( WORDPRESS_PIXELPIN_LOGIN_ABS_PATH ), realpath( WP_PLUGIN_DIR . '/' ), realpath( ABSPATH . '/' ) ) , '', substr( $trace[$i], strpos( $trace[$i], ' ' ) ) );
    }

...
*     do_action('init')
*     .       wpl_process_login()
*     .       .       wpl_process_login_begin()
*     .       .       .       wpl_render_redirect_to_provider_loading_screen()
*     .       .       .       Hybrid_Auth::authenticate()
*     .       .       .       wpl_render_return_from_provider_loading_screen()
*     .       .
*     .       .       wpl_process_login_end()
*     .       .       .       wpl_process_login_get_user_data()
*     .       .       .       .       wpl_process_login_request_user_pixelpin_profile()
*     .       .       .       .       .       Hybrid_Auth::getUserProfile()
*     .       .       .       .
*     .       .       .       .       wpl_process_login_complete_registration()
*     .       .       .
*     .       .       .       wpl_process_login_create_wp_user()
*     .       .       .
*     .       .       .       wpl_process_login_update_wpl_user_data()
*     .       .       .       .       wpl_store_hybridauth_user_profile()
*     .       .       .       .       wpl_buddypress_xprofile_mapping()
*     .       .       .       .       wpl_store_hybridauth_user_contacts()
*     .       .       .
...
*    wp-login.php
*       ?action=wordpress_pixelpin_authenticate                        // current step
*       &mode=login                                                  // auth mode
*       &provider=pixelpin											 // selected provider
...
*/
function wpl_process_login()
{
	// > check for wpl actions
	$action = isset( $_REQUEST['action'] ) ? $_REQUEST['action'] : null;

	if( ! in_array( $action, array( "wordpress_pixelpin_authenticate", "wordpress_pixelpin_profile_completion", "wordpress_pixelpin_account_linking", "wordpress_pixelpin_authenticated" ) ) )
	{
		return false;
	}

	// authentication mode
	$auth_mode = wpl_process_login_get_auth_mode();
	...
	if( 'login' == $auth_mode && is_user_logged_in() )
	{
		...
		return wpl_process_login_render_notice_page( sprintf( _wpl__( "You are already logged in as %s. Do you want to <a href='%s'>log out</a>?", 'wordpress-pixelpin-login' ), $current_user->display_name, wp_logout_url( home_url() ) ) );
	}

	// halt, if mode link and user not logged in
	if( 'link' == $auth_mode && ! is_user_logged_in() )
	{
		return wpl_process_login_render_notice_page( sprintf( _wpl__( "You have to be logged in to be able to link your existing account. Do you want to <a href='%s'>login</a>?", 'wordpress-pixelpin-login' ), wp_login_url( home_url() ) ) );
	}

	// halt, if mode test and not admin
	if( 'test' == $auth_mode && ! current_user_can('manage_options') )
	{
		return wpl_process_login_render_notice_page( _wpl__( 'You do not have sufficient permissions to access this page.', 'wordpress-pixelpin-login' ) );
	}

	// Bouncer :: Allow authentication?
	if( get_option( 'wpl_settings_bouncer_authentication_enabled' ) == 2 )
	{
		return wpl_process_login_render_notice_page( _wpl__( "Authentication through pixelpin networks is currently disabled.", 'wordpress-pixelpin-login' ) );
	}

	add_action( 'wpl_clear_user_php_session', 'wpl_process_login_clear_user_php_session' );

	// HOOKABLE:
	do_action( "wpl_process_login_start" );

	// if action=wordpress_pixelpin_authenticate
	// > start the first part of authentication (redirect the user to the selected provider)
	if( $action == "wordpress_pixelpin_authenticate" )
	{
		return wpl_process_login_begin();
	}

	// if action=wordpress_pixelpin_authenticated or action=wordpress_pixelpin_profile_completion
	// > finish the authentication process (create new user if doesn't exist in database, then log him in within wordpress)
	wpl_process_login_end();
}

add_action( 'init', 'wpl_process_login' );
...
*/
function wpl_process_login_begin()
{
	// HOOKABLE:
	do_action( "wpl_process_login_begin_start" );
	...
	if( ! $provider )
	{
		return wpl_process_login_render_notice_page( _wpl__( 'Bouncer says this makes no sense.', 'wordpress-pixelpin-login' ) );
	}
	...
	if( ! isset( $_REQUEST["redirect_to_provider"] ) )
	{
		...
		return wpl_render_redirect_to_provider_loading_screen( $provider );
	}

	/*  2. Build the hybridauth config for the selected provider (keys, scope, etc) */

	// provider enabled?
	if( ! get_option( 'wpl_settings_' . $provider . '_enabled' ) )
	{
		return wpl_process_login_render_notice_page( _wpl__( "Unknown or disabled provider.", 'wordpress-pixelpin-login' ) );
	}

	$config = wpl_process_login_build_provider_config( $provider );
	...
	// HOOKABLE:
	do_action( "wpl_hook_process_login_before_hybridauth_authenticate", $provider, $config );
	...
	try
	{
		...
		$params = apply_filters("wpl_hook_process_login_authenticate_params",array(),$provider);
	...
	{
		return wpl_process_login_render_error_page( $e, $config, $provider );
	}

	// HOOKABLE:
	do_action( "wpl_hook_process_login_after_hybridauth_authenticate", $provider, $config, $hybridauth, $adapter );
	...
	// build the authenticateD, which will make wpl_process_login() fire the next step wpl_process_login_end()
	$authenticated_url = site_url( 'wp-login.php', 'login_post' ) . ( strpos( site_url( 'wp-login.php', 'login_post' ), '?' ) ? '&' : '?' ) . "action=wordpress_pixelpin_authenticated&provider=" . $provider . '&mode=' . $auth_mode;

	// display a loading screen
	return wpl_render_return_from_provider_loading_screen( $provider, $authenticated_url, $redirect_to, $wpl_settings_use_popup );
}
...
*/
function wpl_process_login_end()
{
	// HOOKABLE:
	do_action( "wpl_process_login_end_start" );

	// HOOKABLE: set a custom Redirect URL
	$redirect_to = wpl_process_login_get_redirect_to();
	...
	if( 'test' == $auth_mode )
	{
		$redirect_to = admin_url( 'options-general.php?page=wordpress-pixelpin-login&wplp=auth-paly&provider=' . $provider );
	...
	if( 'link' == $auth_mode )
	{
		// a pixelpin account cant be associated with more than one wordpress account.

		$hybridauth_user_profile = wpl_process_login_request_user_pixelpin_profile( $provider );

        $adapter = wpl_process_login_get_provider_adapter( $provider );

		$user_id = (int) wpl_get_stored_hybridauth_user_id_by_provider_and_provider_uid( $provider, $hybridauth_user_profile->identifier );

		if( $user_id && $user_id != get_current_user_id() )
		{
			return wpl_process_login_render_notice_page( sprintf( _wpl__( "Your <b>%s ID</b> is already linked to another account on this website.", 'wordpress-pixelpin-login'), $provider ) );
		}
		...
		if( ! $user_id )
		{
			return wpl_process_login_render_notice_page( _wpl__( "Sorry, we couldn't link your account.", 'wordpress-pixelpin-login' ) );
		}
	}
	elseif( 'login' != $auth_mode )
	{
		return wpl_process_login_render_notice_page( _wpl__( 'Bouncer says no.', 'wordpress-pixelpin-login' ) );
	}
	...
	if( 'login' == $auth_mode )
	{
		...
		= wpl_process_login_get_user_data( $provider, $redirect_to );

		// if no associated user were found in wplusersprofiles, create new WordPress user
		if( ! $wordpress_user_id )
		{
			$user_id = wpl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email );
	...
	// if user is found in wplusersprofiles but the associated WP user account no longer exist
	// > this should never happen! but just in case: we delete the user wplusersprofiles/wpluserscontacts entries and we reset the process
	$wp_user = get_userdata( $user_id );

	if( ! $wp_user )
	{
		wpl_delete_stored_hybridauth_user_data( $user_id );

		return wpl_process_login_render_notice_page( sprintf( _wpl__( "Sorry, we couldn't connect you. <a href=\"%s\">Please try again</a>.", 'wordpress-pixelpin-login' ), site_url( 'wp-login.php', 'login_post' ) ) );
	}

	// store user hybridauth profile (wplusersprofiles), contacts (wpluserscontacts) and buddypress mapping
	wpl_process_login_update_wpl_user_data( $is_new_user, $user_id, $provider, $adapter, $hybridauth_user_profile, $wp_user );

	// finally create a wordpress session for the user
	wpl_process_login_authenticate_wp_user( $user_id, $provider, $redirect_to, $adapter, $hybridauth_user_profile, $wp_user );
}
...
*/
function wpl_process_login_get_user_data( $provider, $redirect_to )
{
	// HOOKABLE:
	do_action( "wpl_process_login_get_user_data_start", $provider, $redirect_to );
	...
	/* 1. Grab the user profile from pixelpin network */

	if( ! ( isset( $_SESSION['wpl::userprofile'] ) && $_SESSION['wpl::userprofile'] && $hybridauth_user_profile = json_decode( $_SESSION['wpl::userprofile'] ) ) )
	{
		$hybridauth_user_profile = wpl_process_login_request_user_pixelpin_profile( $provider );

		$_SESSION['wpl::userprofile'] = json_encode( $hybridauth_user_profile );
	}
	...
	if( get_option( 'wpl_settings_bouncer_new_users_restrict_domain_enabled' ) == 1 )
	{
		if( empty( $hybridauth_user_email ) )
		{
			return wpl_process_login_render_notice_page( _wpl__( get_option( 'wpl_settings_bouncer_new_users_restrict_domain_text_bounce' ), 'wordpress-pixelpin-login') );
		}

		$list = get_option( 'wpl_settings_bouncer_new_users_restrict_domain_list' );
	...
	// the user BEFORE we filter by email address
        ...
        if( ! $user_id )
        {
                ...
                if( get_option( 'wpl_settings_bouncer_registration_enabled' ) == 2 )
                {
                        return wpl_process_login_render_notice_page( _wpl__( "Registration is now closed.", 'wordpress-pixelpin-login' ) );
                }
                ...
                if( get_option( 'wpl_settings_bouncer_accounts_linking_enabled' ) == 1 )
                {
                        do
                        {
                                ...
                                = wpl_process_login_new_users_gateway( $provider, $redirect_to, $hybridauth_user_profile );
                        }
                        while( ! $shall_pass );
                ...
                elseif(
                        ...
                        ||
                                get_option( 'wpl_settings_bouncer_profile_completion_change_username' ) == 1
                )
                {
                        do
                        {
	...
	if( get_option( 'wpl_settings_bouncer_new_users_restrict_email_enabled' ) == 1 )
	{
		error_log(__METHOD__ . ' start wpl_settings_bouncer_new_users_restrict_email_enabled.');
		error_log(__METHOD__ . ' hybridauth_user_email is ' . $hybridauth_user_email );
		if( empty( $hybridauth_user_email ) )
		{
			return wpl_process_login_render_notice_page( _wpl__( get_option( 'wpl_settings_bouncer_new_users_restrict_email_text_bounce' ), 'wordpress-pixelpin-login') );
		}

		$list = get_option( 'wpl_settings_bouncer_new_users_restrict_email_list' );
		$list = preg_split( '/$\R?^/m', $list );
	...
	if( get_option( 'wpl_settings_bouncer_new_users_restrict_profile_enabled' ) == 1 )
	{
		error_log(__METHOD__ . ' start restrict_profile_enabled.');
		$list = get_option( 'wpl_settings_bouncer_new_users_restrict_profile_list' );
		$list = preg_split( '/$\R?^/m', $list );
		...
		if( ! $shall_pass )
		{
			return wpl_process_login_render_notice_page( _wpl__( get_option( 'wpl_settings_bouncer_new_users_restrict_profile_text_bounce' ), 'wordpress-pixelpin-login') );
		}
	}

	/* 3. Check if user exist in database by looking for the couple (Provider name, Provider user ID) or verified email */

	// check if user already exist in wplusersprofiles
	$user_id = (int) wpl_get_stored_hybridauth_user_id_by_provider_and_provider_uid( $provider, $hybridauth_user_profile->identifier );

	// if not found in wplusersprofiles, then check his verified email
	if( ! $user_id && ! empty( $hybridauth_user_profile->emailVerified ) )
	{
		// check if the verified email exist in wp_users
		$user_id = (int) wpl_wp_email_exists( $hybridauth_user_profile->emailVerified );

		// the user exists in Wordpress
		$wordpress_user_id = $user_id;

		// check if the verified email exist in wplusersprofiles
		if( ! $user_id )
		{
			$user_id = (int) wpl_get_stored_hybridauth_user_id_by_email_verified( $hybridauth_user_profile->emailVerified );
		}
	}
...
*/
function wpl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email )
{
	// HOOKABLE:
	do_action( "wpl_process_login_create_wp_user_start", $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email );
	...
	if( ! $user_email )
	{
		...
		if( wpl_wp_email_exists ( $user_email ) )
		{
			do
			{
				$user_email = md5( uniqid( wp_rand( 10000, 99000 ) ) ) . '@example.com';
			}
			while( wpl_wp_email_exists( $user_email ) );
		}
	}        
	...
	$wpl_settings_bouncer_new_users_membership_default_role = get_option( 'wpl_settings_bouncer_new_users_membership_default_role' );

	// if level eq "default", we set role to wp default user role
	if( $wpl_settings_bouncer_new_users_membership_default_role == "default" )
	{
		$userdata['role'] = get_option('default_role');
	}

	// if level not eq "default" or 'wplnorole' nor empty, we set role to the selected role in bouncer settings
	elseif( $wpl_settings_bouncer_new_users_membership_default_role && $wpl_settings_bouncer_new_users_membership_default_role != 'wplnorole' )
	{
		$userdata['role'] = $wpl_settings_bouncer_new_users_membership_default_role;
	}
	...
	if( get_option( 'wpl_settings_bouncer_new_users_moderation_level' ) > 100 )
	{
		$userdata['role'] = "pending";
	}

	// HOOKABLE: change the user data
	$userdata = apply_filters( 'wpl_hook_process_login_alter_wp_insert_user_data', $userdata, $provider, $hybridauth_user_profile );

	// DEPRECIATED: as of 2.2.3
	// $userdata = apply_filters( 'wpl_hook_process_login_alter_userdata', $userdata, $provider, $hybridauth_user_profile );

	// HOOKABLE: This action runs just before creating a new wordpress user.
	do_action( 'wpl_hook_process_login_before_wp_insert_user', $userdata, $provider, $hybridauth_user_profile );

	// DEPRECIATED: as of 2.2.3
	// do_action( 'wpl_hook_process_login_before_insert_user', $userdata, $provider, $hybridauth_user_profile );

	// HOOKABLE: This action runs just before creating a new wordpress user, it delegate user insert to a custom function.
	$user_id = apply_filters( 'wpl_hook_process_login_delegate_wp_insert_user', $userdata, $provider, $hybridauth_user_profile );
	...
	if( ! $user_id || ! is_integer( $user_id ) )
	{
		...
		{
			return wpl_process_login_render_notice_page( _wpl__( "An error occurred while creating a new user: ", 'wordpress-pixelpin-login' ) . $user_id->get_error_message() );
		}

		return wpl_process_login_render_notice_page( _wpl__( "An error occurred while creating a new user!", 'wordpress-pixelpin-login' ) );
	}
	...
	// Send notifications
	if( get_option( 'wpl_settings_users_notification' ) == 1 )
	{
		wpl_admin_notification( $user_id, $provider );
	}
	...
	do_action( 'wpl_hook_process_login_after_wp_insert_user', $user_id, $provider, $hybridauth_user_profile );

	// DEPRECIATED: as of 2.2.3
	// do_action( 'wpl_hook_process_login_after_create_wp_user', $user_id, $provider, $hybridauth_user_profile );
...
*/
function wpl_process_login_update_wpl_user_data( $is_new_user, $user_id, $provider, $adapter, $hybridauth_user_profile, $wp_user )
{
	// HOOKABLE:
...
*/
function wpl_process_login_authenticate_wp_user( $user_id, $provider, $redirect_to, $adapter, $hybridauth_user_profile, $wp_user )
{
	// HOOKABLE:
	do_action( "wpl_process_login_authenticate_wp_user_start", $user_id, $provider, $redirect_to, $adapter, $hybridauth_user_profile, $wp_user );

	// update some fields in usermeta for the current user
	update_user_meta( $user_id, 'wpl_current_provider', $provider );

	if(  $hybridauth_user_profile->photoURL )
	{
		update_user_meta( $user_id, 'wpl_current_user_image', $hybridauth_user_profile->photoURL );
	}
	...
	$wpl_settings_bouncer_new_users_moderation_level = get_option( 'wpl_settings_bouncer_new_users_moderation_level' );
	...
	{
		// E-mail Confirmation
		if( $wpl_settings_bouncer_new_users_moderation_level == 101 )
		{
		...
		// Admin Approval
		elseif( $wpl_settings_bouncer_new_users_moderation_level == 102 )
		{
	...
	else
	{
		...
		do_action( "wpl_hook_process_login_before_wp_set_auth_cookie", $user_id, $provider, $hybridauth_user_profile );

		// DEPRECIATED: as of 2.2.3
		// do_action( 'wpl_hook_process_login_before_set_auth_cookie', $user_id, $provider, $hybridauth_user_profile );
	...
	// > To be sure the user is connected, use wpl_hook_process_login_before_wp_set_auth_cookie instead.
	do_action( "wpl_hook_process_login_before_wp_safe_redirect", $user_id, $provider, $hybridauth_user_profile, $redirect_to );
...
*/
function wpl_process_login_build_provider_config( $provider )
{
	$config = array();
	$config["base_url"] = WORDPRESS_PIXELPIN_LOGIN_HYBRIDAUTH_ENDPOINT_URL;
	...
	if( get_option( 'wpl_settings_' . $provider . '_app_id' ) )
	{
		$config["providers"][$provider]["keys"]["id"] = get_option( 'wpl_settings_' . $provider . '_app_id' );
	}

	// provider application key ?
	if( get_option( 'wpl_settings_' . $provider . '_app_key' ) )
	{
		$config["providers"][$provider]["keys"]["key"] = get_option( 'wpl_settings_' . $provider . '_app_key' );
	}

	// provider application secret ?
	if( get_option( 'wpl_settings_' . $provider . '_app_secret' ) )
	{
		$config["providers"][$provider]["keys"]["secret"] = get_option( 'wpl_settings_' . $provider . '_app_secret' );
	}

	// set custom endpoint?
	if( in_array( strtolower( $provider ), array( 'dribbble' ) ) )
	{
		$config["providers"][$provider]["endpoint"] = WORDPRESS_PIXELPIN_LOGIN_HYBRIDAUTH_ENDPOINT_URL . 'endpoints/' . strtolower( $provider ) . '.php';
	}

	// set default scope
	if( get_option( 'wpl_settings_' . $provider . '_app_scope' ) )
	{
		$config["providers"][$provider]["scope"] = get_option( 'wpl_settings_' . $provider . '_app_scope' );
	}

	$provider_scope = isset( $config["providers"][$provider]["scope"] ) ? $config["providers"][$provider]["scope"] : '' ;

	// HOOKABLE: allow to overwrite scopes
	$config["providers"][$provider]["scope"] = apply_filters( 'wpl_hook_alter_provider_scope', $provider_scope, $provider );

	// HOOKABLE: allow to overwrite hybridauth config for the selected provider
	$config["providers"][$provider] = apply_filters( 'wpl_hook_alter_provider_config', $config["providers"][$provider], $provider );
...
/**
*  Grab the user profile from pixelpin network
*/
function wpl_process_login_request_user_pixelpin_profile( $provider )
{
	$adapter                 = null;
	$config                  = null;
	$hybridauth_user_profile = null;
	...
	try
	{
		...
		{
			return wpl_process_login_render_notice_page( sprintf( _wpl__( "Sorry, we couldn't connect you with <b>%s</b>. <a href=\"%s\">Please try again</a>.", 'wordpress-pixelpin-login' ), $provider, site_url( 'wp-login.php', 'login_post' ) ) );
		}
	}
	...
	{
		return wpl_process_login_render_error_page( $e, $config, $provider, $adapter );
	}
...
*/
function wpl_process_login_get_provider_adapter( $provider )
{
	if( ! class_exists( 'Hybrid_Auth', false ) )
	{
		require_once WORDPRESS_PIXELPIN_LOGIN_ABS_PATH . "hybridauth/Hybrid/Auth.php";
...
*
* Note: Widget::Redirect URL can be customised using the filter 'wpl_hook_process_login_alter_redirect_to'
*/
function wpl_process_login_get_redirect_to()
{
	// force redirection?
	$wpl_settings_redirect_url = get_option( 'wpl_settings_redirect_url' );

	if( get_option( 'wpl_settings_force_redirect_url' ) == 1 )
	{
		$redirect_to = apply_filters( 'wpl_hook_process_login_alter_redirect_to', $wpl_settings_redirect_url );
	...
	if( isset( $_REQUEST[ 'redirect_to' ] ) && $_REQUEST[ 'redirect_to' ] != '' )
	{
		...
		{
			$redirect_to = $wpl_settings_redirect_url;
		}
	...
	$redirect_to = apply_filters( 'wpl_hook_process_login_alter_redirect_to', $redirect_to );
...
*/
function wpl_process_login_render_error_page( $e, $config = null, $provider = null, $adapter = null )
{
	// HOOKABLE:
	do_action( "wpl_process_login_render_error_page", $e, $config, $provider, $adapter );

	$assets_base_url  = WORDPRESS_PIXELPIN_LOGIN_PLUGIN_URL . 'assets/img/';

	$message  = _wpl__("Unspecified error!", 'wordpress-pixelpin-login');
	$notes    = "";
	$apierror = substr( $e->getMessage(), 0, 145 );

	switch( $e->getCode() )
	{
		case 0 : $message = _wpl__("Unspecified error.", 'wordpress-pixelpin-login'); break;
		case 1 : $message = _wpl__("WordPress PixelPin Login is not properly configured.", 'wordpress-pixelpin-login'); break;
		...
		case 7 : $message = _wpl__("You're not connected to the provider.", 'wordpress-pixelpin-login'); break;
		case 8 : $message = _wpl__("Provider does not support this feature.", 'wordpress-pixelpin-login'); break;
	}
	...
	return wpl_render_error_page( $message, $notes, $provider, $apierror, $e );
}
...
*/
function wpl_process_login_render_notice_page( $message )
{
	// HOOKABLE:
	do_action( "wpl_process_login_render_notice_page", $message );

	return wpl_render_notice_page( $message );
}
...
*/
function wpl_process_login_get_selected_provider()
{
	$provider = isset( $_REQUEST["provider"] ) ? sanitize_text_field( $_REQUEST["provider"] ) : null;

	return apply_filters( 'wpl_hook_process_login_alter_provider', $provider ) ;
}
...
*/
function wpl_process_login_get_auth_mode()
{
	$auth_mode = isset( $_REQUEST["mode"] ) ? sanitize_text_field( $_REQUEST["mode"] ) : 'login';

	return apply_filters( 'wpl_hook_process_login_alter_auth_mode', $auth_mode ) ;
}

// --------------------------------------------------------------------

/**
* Clear the stored data by hybridauth and wpl in php session
*/
function wpl_process_login_clear_user_php_session()
{
	...
	$_SESSION["wpl::userprofile"] = array(); // used by wpl to temporarily store the user profile so we don't make unnecessary calls to pixelpin apis.
}
...
*/
function wpl_get_provider_name_by_id( $provider_id)
{
        global $WORDPRESS_PIXELPIN_LOGIN_PROVIDERS_CONFIG;

        foreach( $WORDPRESS_PIXELPIN_LOGIN_PROVIDERS_CONFIG as $provider_settings ) {

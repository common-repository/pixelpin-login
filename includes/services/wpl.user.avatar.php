...
/** 
* Display users avatars from pixelpin networks when available
* 
* Note: 
*   You may redefine this function
*/
if( ! function_exists( 'wpl_get_wp_user_custom_avatar' ) )
{
	function wpl_get_wp_user_custom_avatar($html, $mixed, $size, $default, $alt)
	{
		...
		if( $user_id )
		{
			$wpl_avatar = wpl_get_user_custom_avatar( $user_id );

			if( $wpl_avatar )
			{
				$wpl_html = '<img alt="'. $alt .'" src="' . $wpl_avatar . '" class="avatar avatar-wordpress-pixelpin-login avatar-' . $size . ' photo" height="' . $size . '" width="' . $size . '" />';

				// HOOKABLE: 
				return apply_filters( 'wpl_hook_alter_wp_user_custom_avatar', $wpl_html, $user_id, $wpl_avatar, $html, $mixed, $size, $default, $alt );
			}
		}
...
add_filter( 'get_avatar', 'wpl_get_wp_user_custom_avatar', 10, 5 );

// --------------------------------------------------------------------

/**
* Display users avatars from pixelpin networks on buddypress
* 
* Note: 
*   You may redefine this function
*/
if( ! function_exists( 'wpl_get_bp_user_custom_avatar' ) )
{
	function wpl_get_bp_user_custom_avatar($html, $args)
	{
		...
		if( is_array( $args ) )
		{
			...
			if( ! empty( $args['object'] ) AND strtolower( $args['object'] ) == 'user' )
			{
				...
				if( ! empty( $args['item_id'] ) AND is_numeric( $args['item_id'] ) )
				{
					...
					if( $wpl_avatar )
					{
						$img_class  = ('class="' .(!empty($args ['class']) ?($args ['class'] . ' ') : '') . 'avatar-wordpress-pixelpin-login" ');
						...
						//Replace
						$wpl_html = preg_replace('#<img[^>]+>#i', '<img src="' . $wpl_avatar . '" ' . $img_alt . $img_class . $img_height . $img_width . '/>', $html );

						// HOOKABLE: 
						return apply_filters( 'wpl_hook_alter_get_bp_user_custom_avatar', $wpl_html, $user_id, $wpl_avatar, $html, $args );
					}
				}
			}
		}
...
add_filter( 'bp_core_fetch_avatar', 'wpl_get_bp_user_custom_avatar', 10, 2 );
